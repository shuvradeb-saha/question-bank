...
class QuestionStatusManagement extends Component {
  static propTypes = {
  ...
  componentDidMount() {
    ...
    if (type === QuestionStatusType.PENDING) fetchPendingList(eiin);
    if (type === QuestionStatusType.APPROVED) fetchApprovedList(eiin);
  }
  ...
  render() {
    return (
      <div>
        {this.props.type === QuestionStatusType.APPROVED && (
          <this.ApprovedInfo />
        )}
        {this.props.type === QuestionStatusType.PENDING && <this.PendingInfo />}
...
export default compose(withConnect)(QuestionStatusManagement);
